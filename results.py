...
class ResultWriter(Worker):
    def __init__(self, config, testcase, results_queue, total_data, valid_start_time, valid_end_time,
                 current_concurrency):
        ...
        self.totalThreads = self.config['Concurrency']
    ...
    def run(self):
        ...
        while True:
            ...
            if self.current_concurrency.value == -2 or (self.current_concurrency.value == -1 and self.results_queue.empty()):
                # 退出前将未写入的记录写入
                if self.config['RecordDetails']:
                    self.detailWriterLogger.info('\n'.join(writer_buffer))
    ...
    def upgrade_and_print_result(self, q_tuple):
        ...
        if not q_tuple:
            current_time = time.time()
            if current_time - self.lastUpdateTime > self.refresh_period:
        ...
        if self.testcase == 'PutObject':
            self.already_transferred_data += data_send
        elif self.testcase == 'GetObject':
        ...
        if self.total_data.value > 0:
            self.currentProgressPercent = (self.already_transferred_data / self.total_data.value) * 100
        ...
        if (start_time >= self.valid_start_time.value) and (end_time <= self.valid_end_time.value):
            ...
            if not self.hasThreadQuit:
                logging.warn("runThreads: %d" % self.current_concurrency.value)
        ...
        # 性能结果统计和打印结果到屏幕有条件刷新
        if (time.time() - self.lastUpdateTime < self.refresh_period) \
    ...
    def print_progress(self):
        ...
        total_result_arr = [proc_bar.ljust(100),
                            '%-22s%-20s RunTime: %-52s' % ('[Operation]', self.testcase, run_time),
