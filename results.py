...
class ResultWriter(Process):
    def __init__(self, config, test_case, results_queue, total_data, valid_start_time, valid_end_time,
                 current_concurrency):
        ...
        detail_result_file = time.strftime('result/%Y.%m.%d_%H.%M.%S', time.localtime()) + '_' + test_case + '_' + str(
            self.config['Concurrency']) + '_detail.csv'
        detail_writer_handler = logging.handlers.RotatingFileHandler(detail_result_file, mode='a',
                                                                     maxBytes=1024 * 1024 * 1024,
                                                                     backupCount=5, encoding=None, delay=0)
        self.detail_writer_logger = logging.getLogger("detailWriter")
        self.detail_writer_logger.propagate = 0
        self.detail_writer_logger.addHandler(detail_writer_handler)
        self.detail_writer_logger.setLevel(logging.INFO)
        ...
        realtime_writer_handler = logging.handlers.RotatingFileHandler(realtime_perf_file, mode='a',
                                                                       ...
                                                                       backupCount=5, encoding=None, delay=0)
        self.realtime_writer_logger = logging.getLogger("realtimeWriter")
        self.realtime_writer_logger.propagate = 0
        self.realtime_writer_logger.addHandler(realtime_writer_handler)
        self.realtime_writer_logger.setLevel(logging.INFO)
        ...
        if total_data == -1:
            ...
            self.total_data = total_data  # 总字节数

        self.valid_start_time = valid_start_time  # 所有并发均存在的最早时间。
        self.valid_end_time = valid_end_time  # 结束时间。
        self.last_end_time = 0.0  # 上一次计算tps的时间。
        self.last_update_time = 0  # 上一次实时刷新时间
        self.no_q_refresh = True
        self.current_progress_percent = 0.0  # 当前执行进展

        self.refresh_period = 2
        self.execute_path = os.getcwd()

        self.progress_latency = []
        self.has_worker_quit = False
        ...
        # totalInnerFlowControl     整个测试过程服务器端上内部流控错误
        ...
        self.http_status_dict = {}  # 记录所有http状态响应码数量
        self.current_concurrency = current_concurrency  # 当前运行的用户
        self.refreshed_once = False
        self.last10_realtime_stat = {}  # 如存最新10次的统计结果。
        self.last_serial_no = 1
        self.already_transferred_data = 0
        self.speed = 0
    ...
    def __writeRealTimeFile__(self, statistic_item):
        ...
        while self.last_serial_no < statistic_item.serial_no:
            ...
            self.realtime_writer_logger.info(tmp_statistic_item.to_string())
            self.last_serial_no += 1
        self.realtime_writer_logger.info(statistic_item.to_string())
    ...
    def run(self):
        ...
        def clear_err_statics(signal_num, e):
            logging.warn('signal_num %d %r received, to clear err statics' % (signal_num, e))
            # 清除错误请求数
            if signal_num == 10:
                logging.warn('clear error stats signal 10 received')
                for key in self.http_status_dict.keys():
                    if key >= '400':
                        self.http_status_dict[key] = 0
        ...
        while True:
            # self.current_concurrency.value == -2代表测试被意外中止，-1代表用户进程已全部结束。
            if self.current_concurrency.value == -2 or \
                    (self.current_concurrency.value == -1 and self.results_queue.empty()):
                # 退出前将未写入的记录写入
                if self.config['RecordDetails']:
            ...
            try:
                # 从Queue中取记录
                q_tuple = self.results_queue.get(block=True, timeout=2)
                # 写记录到文件
                if self.config['RecordDetails']:
                ...
                logging.warn('get record from queue exception: %s ' % data)
                time.sleep(.01)
            finally:
                # 刷新当前进展到内存和屏幕
    ...
    def upgrade_and_print_result(self, q_tuple):
        ...
        if not q_tuple:
            current_time = time.time()
            if current_time - self.last_update_time > self.refresh_period:
        ...
        if self.test_case == 'PutObject':
            self.already_transferred_data += data_send
        elif self.test_case == 'GetObject':
            self.already_transferred_data += data_recv

        # 若totalRequests == -1 表明无法计算当前进度。total_data ==0 表明操作类型暂时无法计算当前进度。
        if self.total_data.value > 0:
            self.current_progress_percent = (self.already_transferred_data / self.total_data.value) * 100

        if self.current_progress_percent > 100.0:
            self.current_progress_percent = 100.0

        if end_time > self.valid_end_time.value:
            if not self.has_worker_quit:
                logging.warn("Concurrency: %d" % self.current_concurrency.value)
                self.has_worker_quit = True

        # 2.刷新内存中的结果
        # httpStatus_dict
        # 没有响应，记为'',否则对应的响应加1
        http_status = status.split(' ')[0]
        if http_status in self.http_status_dict:
            self.http_status_dict[http_status] += 1
        else:
            self.http_status_dict[http_status] = 1
        # rough_summary_dict
        if status < '400':
        ...
        if status < '400' or self.config['BadRequestCounted']:
            ...
            # if self.config['LatencyPercentileMap']:
            self.progress_latency.append(latency)
        ...
        if self.config['StatisticsInterval'] > 0 and not self.config['CollectBasicData']:
            ...
            # 3. 如果当前归属的序号不在已统计的结果，并且实时结果中=10个
                ...
                if serial_no < sorted(self.last10_realtime_stat)[0]:
                    logging.warn('record is ignored. %s' % str(q_tuple))
                else:
                    # 初始化一个新的序号结果，并刷新
                    self.last10_realtime_stat[serial_no] = StatisticItem(serial_no, self.valid_start_time.value,
                                                                         self.config['StatisticsInterval'],
        ...
        # 性能结果统计和打印结果到屏幕有条件刷新
        if (time.time() - self.last_update_time < self.refresh_period) \
                and self.total_data.value != float(self.already_transferred_data) \
                and not self.no_q_refresh:
            return
        ...
        if self.test_case == 'PutObject':
            self.speed = self.accurate_summary_dict['sendBPS']
        elif self.test_case == 'GetObject':
            self.speed = self.accurate_summary_dict['recvBPS']

        self.last_update_time = time.time()
        if self.config['PrintProgress']:
            self.print_progress()
        self.no_q_refresh = False

    def print_progress(self):
        if self.current_concurrency.value == -2:
            return
        last5_static = ''
        if self.config['StatisticsInterval'] > 0 and not self.config['CollectBasicData']:
            # 去除第一个值，正在统计,可能不准确。
        ...
        if self.speed <= 0.0:
            ...
            time_left_str = '  EST:' + self.convert_time_format_str(
                float(self.total_data.value - self.already_transferred_data) / self.speed)
            logging.debug("time_left_str:%s" % time_left_str)

        num_hashes = int(round(self.current_progress_percent) / 100.0 * 70)
        proc_bar = '[' + '#' * num_hashes + '-' * (70 - num_hashes) + ']'
        if self.current_progress_percent < 0.1:
            percent_place = (len(proc_bar) / 2) - 4
            percent_string = '<0.1%'
        else:
            percent_place = (len(proc_bar) / 2) - len("{0:.1f}".format(self.current_progress_percent))
            percent_string = "{0:.1f}".format(self.current_progress_percent) + '%'
        ...
        client_err_str = ' '.join(
            ...
            ' ', '')
        server_err_str = ' '.join(
            [str((k, v)) for (k, v) in self.http_status_dict.items() if '500' <= k < '900']).replace('\'',
                                                                                                     '').replace(
            ' ', '')
        unknown_err_str = ' '.join([str((k, v)) for (k, v) in self.http_status_dict.items() if k >= '900']).replace(
            '\'',
            '').replace(
            ' ', '')
        if self.current_concurrency.value < 0:
            ...
            current_concurrency = self.current_concurrency.value
        run_time_origin = self.last_update_time - self.valid_start_time.value
        ...
        total_result_arr = [proc_bar.ljust(100),
                            '%-22s%-20s RunTime: %-52s' % ('[Operation]', self.test_case, run_time),
                            '%-22s%-82d' % ('[RunningConcurrency]', current_concurrency),
                            '%-22s%-82d' % ('[Requests]', self.rough_summary_dict['currentRequests']),
                            '%-22s%-9d%-73s' % ('[OK]', self.rough_summary_dict['totalOK'], ok_str)]

        if not self.config['CollectBasicData']:
            if self.rough_summary_dict['totalClientErr'] > 0:
                total_result_arr.append('%-22s\033[1;31;40m%-9d%-73s\033[0m\033[0;32;40m' % (
                    '[ClientErrs]', self.rough_summary_dict['totalClientErr'], client_err_str))
            else:
                total_result_arr.append('%-22s%-82d' % ('[ClientErrs]', self.rough_summary_dict['totalClientErr']))
            if self.rough_summary_dict['totalServerErr'] > 0:
                total_result_arr.append('%-22s\033[1;31;40m%-9d%-73s\033[0m\033[0;32;40m' % (
                    '[ServerErrs]', self.rough_summary_dict['totalServerErr'], server_err_str))
            else:
                total_result_arr.append('%-22s%-82d' % ('[ServerErrs]', self.rough_summary_dict['totalServerErr']))
            if self.rough_summary_dict['totalOuterFlowControl'] > 0:
                total_result_arr.append('%-22s\033[1;31;40m%-82d\033[0m\033[0;32;40m' % (
                    ' -OuterFlwCtr', self.rough_summary_dict['totalOuterFlowControl']))
            else:
                total_result_arr.append(
                    '%-22s%-82d' % (' -OuterFlwCtr', self.rough_summary_dict['totalOuterFlowControl']))
            if self.rough_summary_dict['totalInnerFlowControl'] > 0:
                total_result_arr.append('%-22s\033[1;31;40m%-82d\033[0m\033[0;32;40m' % (
                    ' -InnerFlwCtr', self.rough_summary_dict['totalInnerFlowControl']))
            else:
                total_result_arr.append(
                    '%-22s%-82d' % (' -InnerFlwCtr', self.rough_summary_dict['totalInnerFlowControl']))
            if self.rough_summary_dict['totalOtherErr'] > 0:
                total_result_arr.append('%-22s\033[1;31;40m%-9d%-73s\033[0m\033[0;32;40m' % (
                    '[OtherErrs]', self.rough_summary_dict['totalOtherErr'], unknown_err_str))
            else:
                total_result_arr.append('%-22s%-82d' % ('[OtherErrs]', self.rough_summary_dict['totalOtherErr']))

        error_rate = '%.2f %%' % self.rough_summary_dict['errorRate']
        if self.rough_summary_dict['errorRate'] > 0.0:
            total_result_arr.append('%-22s\033[1;31;40m%-82s\033[0m\033[0;32;40m' % ('[ErrRate]', error_rate))
        else:
            total_result_arr.append('%-22s%-82s' % ('[ErrRate]', error_rate))
        if self.accurate_summary_dict['tps'] <= 0.0:
            total_result_arr.append('%-22s%-82s' % ('[TPS*]', '--'))
        else:
            total_result_arr.append('%-22s%-82.2f' % ('[TPS*]', self.accurate_summary_dict['tps']))

        if not self.config['CollectBasicData']:
            if last5_static == '':
                last5_static = '--'
            total_result_arr.append('%-22s%-82s' % ('[Last5TPS*]', last5_static))

        if self.accurate_summary_dict['avgLatency'] <= 0.0:
            total_result_arr.append('%-22s%-82s' % ('[AvgLatency*]', '--'))
        else:
            total_result_arr.append(
                '%-22s%-82s' % ('[AvgLatency*]', self.convert_time_str(self.accurate_summary_dict['avgLatency'])))

        if not self.config['CollectBasicData']:
            if self.accurate_summary_dict['latencyPercentDescription'] == '':
                self.accurate_summary_dict['latencyPercentDescription'] = '--'
            total_result_arr.append(
                '%-22s%-82s' % ('[latencyPercent*]', self.accurate_summary_dict['latencyPercentDescription']))

            if self.rough_summary_dict['roughTotalSendBytes'] <= 0:
                total_result_arr.append('%-22s%-82s' % ('[DataSend]', '--'))
            else:
                total_result_arr.append(
                    '%-22s%-82s' % (
            ...
            if self.rough_summary_dict['roughTotalRecvBytes'] <= 0:
                total_result_arr.append('%-22s%-82s' % ('[DataRecv]', '--'))
            else:
                total_result_arr.append(
                    '%-22s%-82s' % (
        ...
        if self.accurate_summary_dict['sendBPS'] <= 0:
            total_result_arr.append('%-22s%-82s' % ('[SendThroughput*]', '--'))
        else:
            send_bps = self.convert_to_size_str(self.accurate_summary_dict['sendBPS']) + '/s'
            total_result_arr.append(
                '%-22s%-82s' % ('[SendThroughput*]', send_bps))
        if self.accurate_summary_dict['recvBPS'] <= 0:
            total_result_arr.append('%-22s%-82s' % ('[RecvThroughput*]', '--'))
        else:
            recv_bps = self.convert_to_size_str(self.accurate_summary_dict['recvBPS']) + '/s'
            total_result_arr.append('%-22s%-82s' % ('[RecvThroughput*]', recv_bps))

        if not self.config['CollectBasicData']:
            total_result_arr.append('%-21s%s%-49s' % ('[BestReq*]', 'RequestID'.center(34), 'Latency(s)'))
            for key in sorted(self.accurate_summary_dict['bestRequests']):
                total_result_arr.append('%-21s%s%-49s' % (
            ...
            for i in range(3 - len(self.accurate_summary_dict['bestRequests'])):
                total_result_arr.append('%-21s%s%-49s' % (' ', '-'.center(34), '-'))
        ...
        total_result_arr.append('%-22s%-82s' % ('[ExecutePath]', self.execute_path))
    ...
    def generate_write_final_result(self):
        ...
        if self.config['LatencyPercentileMap'] and self.progress_latency is not None and len(self.progress_latency) > 0:
        ...
        self.accurate_summary_dict['requestsWhenThreadQuit'] = len(self.progress_latency)
        ...
        if self.config['LatencyRequestsNumber']:
            if self.progress_latency is not None and len(self.progress_latency) > 0:
                ...
                dis_first_part = avg_latency - self.progress_latency[0]
                dis_second_part = self.progress_latency[len(self.progress_latency) - 1] - avg_latency
                ...
                for i in range(piece_number):
                    if i == piece_number - 1:
                        interval_max = self.progress_latency[len(self.progress_latency) - 1]
        ...
        unknown_err_str = ' '.join([str((k, v)) for (k, v) in self.http_status_dict.items() if k >= '900']).replace(
            ...
            ' ', '')
        if not self.config['CollectBasicData']:
            total_result = proc_bar + '\n' + \
                           '[Operation]         ' + self.test_case + '\n' + \
                           '[RunTime]           ' + run_time + '\n' + \
        ...
        if self.config['LatencyRequestsNumber'] and latency_requests_number_map is not None and \
                latency_requests_number_list is not None:
    ...
    def generate_latency_percentile_map_description(self):
        ...
        for key in sorted(self.accurate_summary_dict['latencyPercentileMapSections']):
            latency_index = int(round(
                len(self.progress_latency) * int(key) / 100) - 1)
            latency_index = latency_index if latency_index >= 0 else 0
            latency_percentile_map_result = round(self.progress_latency[latency_index] * 1000, 3)
    ...
    def archive_result(self):
        ...
        config_keys = ['DomainName', 'Concurrency', 'VirtualHost', 'IsHTTPs', 'ConnectTimeout', 'BucketNameFixed',
                       'RecordDetails', 'BadRequestCounted']
        config_str = ''
        for item in config_keys:
            if item in self.config:
                config_str += str(self.config[item]).replace(',', ';').replace('\n', ';').replace('\r', '').replace(
                    ...
                    '{', '').replace('}', '')
            # 对上传操作补充输出对象大小
            elif item == 'ObjSize' and self.config['Testcase'] == 201 and not self.config['IsFromLocal']:
            ...
            try:
                ...
                if not os.path.exists(self.archive_file):
                    ...
                    archive_str = 'Start_At,End_At,TestCase,%s,%s\n%s,%s,%s,%s%s\n' % (
                        ...
                        str(self.valid_end_time.value), self.test_case,
                        config_str, result_str)
                    archive_file_obj.write(archive_str)
                else:
                    archive_file_obj = open(self.archive_file, 'a')
                    archive_str = '%s,%s,%s,%s%s\n' % (
                        str(self.valid_start_time.value), str(self.valid_end_time.value), self.test_case, config_str,
                        result_str)
                    archive_file_obj.write(archive_str)
                logging.info('Archive result:[%r]' % archive_str)
                break
            except Exception, e:
                logging.warn('time %d , open and write file %s error: %s' % (i, self.archive_file, e))
                time.sleep(.5)
                continue
            finally:
                try:
                    ...
                    logging.warn('%d close file %s error: %s' % (i, self.archive_file, e))
...
class StatisticItem:
    ...
    def to_string(self):
        ...
        return (str(self.serial_no)).ljust(8) + self.start_time.ljust(20) + str(self.totalOK).ljust(12) + str(
            self.totalRequests).ljust(12) + str(err_rate).ljust(12) + self.tps.ljust(10) + str(
            avg_latency).ljust(13) + ' ' + str(self.totalSend).ljust(16) + ' ' + str(self.totalRecv).ljust(16)